...
jQuery(document).ready(function($) {
        ...
        cdekShippingLabels.each(function() {
            ...
            if (deliveryCost === 0) {
                ...
                if (point.name.includes('Самовывоз')) {
                    ...
                    // Добавляем описание для самовывоза если есть
                    if (point.description) {
                        newText += '<br><small style="color: #666; font-weight: normal;">' + point.description + '</small>';
                    }
                } else if (point.name.includes('менеджером')) {
                    newText = '📞 ' + point.name + ' - Бесплатно';
                    // Добавляем описание для менеджера если есть
                    if (point.description) {
                        newText += '<br><small style="color: #666; font-weight: normal;">' + point.description + '</small>';
                    }
                } else {
    ...
    window.updateShippingTextForPickup = function() {
        ...
        updateClassicShippingCost({
            name: 'Самовывоз (г.Саратов, ул. Осипова, д. 18а)',
            description: 'Заберите заказ самостоятельно по адресу: г. Саратов, ул. Осипова, д. 18а. Режим работы: Пн-Пт 9:00-18:00, Сб 10:00-16:00.'
        }, 0);
    ...
    window.updateShippingTextForManager = function() {
        ...
        // Обновляем отображение в чекауте
        updateClassicShippingCost({
            name: 'Обсудить доставку с менеджером',
            description: 'Наш менеджер свяжется с вами для обсуждения удобного способа доставки: СДЭК, Почта России, Яндекс.Доставка, курьерская доставка по городу или другие варианты по договоренности.'
        }, 0);
