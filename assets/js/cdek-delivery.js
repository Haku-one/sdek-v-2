 ...
 * Используются современные библиотеки: axios, lodash, moment
...
// Настройка axios для WordPress
axios.defaults.timeout = 30000;
axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';

// ========== СОВРЕМЕННЫЕ УТИЛИТЫ БЕЗ КЭШИРОВАНИЯ ==========
...
class SmartAddressSearch {
    constructor() {
        // Убрано кэширование - всегда свежие результаты
    ...
    performSearch(query, callback) {
        ...
        // Убрано кэширование - всегда выполняем свежий поиск
...
jQuery(document).ready(function($) {
    ...
    // Инициализируем современные утилиты без кэширования
    ...
    function calculateDeliveryCost(point, callback) {
        ...
        // Современный HTTP клиент с axios
        const requestData = new FormData();
        requestData.append('action', 'calculate_cdek_delivery_cost');
        requestData.append('point_code', point.code);
        requestData.append('point_data', JSON.stringify(point));
        requestData.append('cart_weight', cartData.weight);
        requestData.append('cart_dimensions', JSON.stringify(cartData.dimensions));
        requestData.append('cart_value', cartData.value);
        requestData.append('has_real_dimensions', cartData.hasRealDimensions ? 1 : 0);
        requestData.append('packages_count', cartData.packagesCount || 1);
        requestData.append('nonce', cdek_ajax.nonce || '');

        axios.post(cdek_ajax.ajax_url, requestData, {
            timeout: 30000,
            headers: {
        ...
        .then(response => {
            ...
            console.log('Ответ API расчета стоимости:', data);
            
            if (data && data.success && data.data && data.data.delivery_sum) {
                let deliveryCost = parseInt(data.data.delivery_sum);
                
                if (cartData.packagesCount > 1) {
                    const costPerPackage = deliveryCost;
                ...
                if (data.data.fallback) {
                    ...
                    console.log('Причина:', data.data.message);
                } else if (data.data.api_success) {
                    console.log('✅ Успешно получена стоимость из настоящего API СДЭК:', deliveryCost, 'руб.');
                    if (data.data.alternative_tariff) {
                        console.log('Использован альтернативный тариф:', data.data.alternative_tariff);
                    }
                } else {
                ...
                console.error('❌ API СДЭК вернул ошибку:', data.data ? data.data.message : 'Неизвестная ошибка');
                
                // Используем fallback вместо показа ошибки пользователю
                const fallbackCost = calculateFallbackCost(point, cartData);
        ...
        function saveRecentSearch(suggestion) {
            try {
                ...
                recentSearches = recentSearches.filter(item => item.city !== suggestion.city);
                        ...
                        timestamp: moment().valueOf() // Используем moment.js для работы с временем
                    });
    ...
    function performCdekSearch() {
        ...
        // Современный HTTP клиент с axios для получения пунктов выдачи
        const requestData = new FormData();
        requestData.append('action', 'get_cdek_points');
        requestData.append('address', searchAddress);
        requestData.append('nonce', cdek_ajax.nonce);
        ...
        .then(response => {
            ...
            if (data.success && data.data) {
                console.log('✅ Получено ПВЗ от API:', data.data.length);
                ...
                if (data.data.length > 0) {
                    ...
                    const tyumenPoints = data.data.filter(point => {
                        let hasZelinsky = false;
                ...
                displayCdekPoints(data.data);
            } else {
                console.error('❌ Ошибка получения ПВЗ:', data);
                showPvzError('Не удалось загрузить пункты выдачи');
            }
        })
        .catch(error => {
            hidePvzLoader();
            console.error('Ошибка получения пунктов СДЭК:', {
                message: error.message,
                response: error.response?.data,
                status: error.response?.status
            });
    ...
    function displayCdekPoints(points) {
        ...
        if (bounds.length > 0) {
            if (bounds.length === 1) {
                ...
                // Используем lodash для более читаемого кода
                const lats = _.map(bounds, coord => coord[0]);
                const lons = _.map(bounds, coord => coord[1]);
                const minLat = _.min(lats);
                const maxLat = _.max(lats);
                const minLon = _.min(lons);
                const maxLon = _.max(lons);
    ...
    function hideCdekShippingBlock() {
                    // Используем moment.js для проверки времени последнего вызова
            const now = moment().valueOf();
            if (window.lastHideCall && (now - window.lastHideCall) < 1000) return;
            window.lastHideCall = now;
        ...
        cdekInputs.each(function() {
            ...
            var packageElement = $this.closest('.wc-block-components-shipping-rates-control__package');
            var control = $this.closest('.wc-block-components-shipping-rates-control');
            var label = $this.closest('label');
            
            if (radioControl.length) radioControl.hide();
            if (packageElement.length) packageElement.hide();
