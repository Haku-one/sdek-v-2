...
jQuery(document).ready(function($) {
    ...
    function selectCdekPoint(point) {
        ...
        }, 500); // Даем время для обновления DOM
    ...
    window.updateShippingTextForManager = function() {
        ...
        // Ищем блоки доставки
        var shippingBlocks = $('.wc-block-components-totals-shipping .wc-block-components-totals-item, .wp-block-woocommerce-checkout-order-summary-shipping-block .wc-block-components-totals-item');
        ...
        shippingBlocks.each(function() {
            ...
            var $description = $block.find('.wc-block-components-totals-item__description');
            
            if ($label.length) {
                $label.text('Менеджер');
                $value.html('<strong>Бесплатно</strong>');
                $description.empty();
                console.log('✅ Обновлен текст доставки на "Менеджер"');
            }
        });
    }
    ...
    window.updateShippingTextForPickup = function() {
        ...
        shippingBlocks.each(function() {
            ...
            if ($label.length) {
                $label.text('Самовывоз (г.Саратов, ул. Осипова, д. 18а)');
                $value.html('<strong>Бесплатно</strong>');
                $description.empty();
                console.log('✅ Восстановлен стандартный текст самовывоза');
            }
        });
    }
    ...
    function initDiscussDeliveryTab() {
        ...
        Utils.hide(Utils.select('.wc-block-components-validation-error', true));
        Utils.select('.has-error', true).forEach(el => el.classList.remove('has-error'));
    }
    
            // Функция добавления вкладки "Обсудить доставку"
        function addDiscussTab() {
            const container = Utils.select('.wc-block-checkout__shipping-method-container');
